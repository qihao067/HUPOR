...
class DepthLoss(nn.Module):
    ...
    def forward(self, output, rdepth):
        ...
        loss, count = 0., 0
        loss_rel, loss_abs = 0.0, 0.0   
        for i in range(batch_size):
            for j in range(len(rdepth[i])):
                for k in range(output_num):
                    if rdepth[i, k, j, 2] > 0:
                        loss_abs += torch.abs(output[i, k, int(rdepth[i][k][j][0]), int(rdepth[i][k][j][1])] - rdepth[i][k][j][2])
                        count += 1
        loss = loss_abs

        if count == 0:   # used for forward
            loss += torch.abs(output[0, 0, 1, 1] - rdepth[0][0][0][2])
            loss = loss * 0
            count = 1
