...
from dataset.infer_dataset import SyntheticDataset
...
def get_train_loader(cfg, num_gpu, is_dist=True, is_shuffle=True, start_iter=0, 
    ...
    if cfg.DATASET.NAME == 'MIX':
        raise NameError("This model is only used for training InferNet. Dataset is not defined!", cfg.DATASET.NAME)
    elif cfg.DATASET.NAME == 'INFER':
