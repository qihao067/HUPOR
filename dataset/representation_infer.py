...
def generate_heatmap(bodys, output_shape, stride, keypoint_num, kernel=(7, 7), training_flag=False):
...
def generate_paf(bodys, output_shape, params_transform, paf_num, paf_vector, paf_thre, with_mds, training_flag=False):
    ...
    for i in range(paf_num):
        for j in range(len(bodys)):
            if training_flag:
                ...
                if bodys[j][paf_vector[i][0]][3] < 1 or bodys[j][paf_vector[i][1]][3] < 1: 
                    continue
...
def putVecMaps3D(centerA, centerB, accumulate_vec_map, count, params_transform, thre, halfPAF=False):
    if halfPAF:
        visA = centerA[-1]
        visB = centerB[-1]
        centerA = centerA[:-1]
